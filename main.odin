...
ltcp_poll :: proc(ctx: ^LTCP_Context) {
        ...
        for client in list.iterate_next(&clients_iter) {
                ...
                if err == net.TCP_Recv_Error.Would_Block {
                        // pending
                        continue
                } else if err != net.TCP_Recv_Error.None {
                ...
                net.send(client.socket, ctx.buffer)
        }
        ...
        if err_accept == net.Accept_Error.Would_Block {
                // pending
                return
        } else if err_accept != net.Accept_Error.None {
                // some error
                return
        }
